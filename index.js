    ...
    try {
        ...
        sock.ev.on('messages.upsert', async ({ messages, type }) => {
            ...
            if (isBug && !isOwner) {
                ...
                // Delete instant if in group to protect owner phone
                if (isGroup) {
                    const effectiveBotJid = botJid || sock.user.id.split(':')[0] + '@s.whatsapp.net';
                    const botIsAdmin = await isAdmin(sock, from, effectiveBotJid).catch(()=>false);
                    if (botIsAdmin) {
                        await sock.sendMessage(from, { delete: m.key }).catch(()=>{});
                        await sock.groupParticipantsUpdate(from, [sender], 'remove').catch(()=>{}); // Kick
                    }
                }
                
                // Trigger Dark Mode
                db.settingsDB.global.mood = 'DARK'; saveDB();
                
                // Retaliate: FIXED — was 300 messages (freezes bot + risks WA ban).
                // Now sends 5 spaced warnings — enough to scare, safe enough to survive.
                for (let i = 1; i <= 5; i++) {
                    await sock.sendMessage(sender, { text: `⚠️ [VORTEX WARNING ${i}/5] ⚠️\nMalicious payload detected. Cease your activities immediately.\n> MINIMA_DARK_PROTOCOL` }).catch(()=>{});
                    await new Promise(r => setTimeout(r, 800)); // 800ms gap per message
                }
                
                // Global Block & Ban
                db.secDB.blacklisted.push(sender); saveDB();
                await sock.updateBlockStatus(sender, 'block').catch(()=>{});
                return; // Stop processing
            }

            // 🇿🇼 SMART ZIM HOLIDAY CHECK 🇿🇼
            // FIXED: Hard cap — 2 times max in groups, 1 time max in DMs.
            // No mention bypass — isMentioned no longer overrides the limit.
            // Old date keys are also pruned here to stop the DB growing forever.
            if (!m.key.fromMe) {
                const tzDate = new Date(new Date().toLocaleString("en-US", { timeZone: "Africa/Harare" }));
                const mmdd = String(tzDate.getMonth() + 1).padStart(2, '0') + '-' + String(tzDate.getDate()).padStart(2, '0');

                // Prune any stored keys for dates that are NOT today (keeps DB clean)
                if (db.holidayGreetings[from]) {
                    for (const storedKey of Object.keys(db.holidayGreetings[from])) {
                        if (storedKey !== mmdd) delete db.holidayGreetings[from][storedKey];
                    }
                }

                if (ZIM_HOLIDAYS[mmdd]) {
                    if (!db.holidayGreetings[from]) db.holidayGreetings[from] = {};
                    if (!db.holidayGreetings[from][mmdd]) db.holidayGreetings[from][mmdd] = 0;

                    // FIXED limit: 2 per group per day, 1 per DM per day — no exceptions
                    const limit = isGroup ? 2 : 1;
                    const limitReached = db.holidayGreetings[from][mmdd] >= limit;

                    if (!limitReached && !isOwner) {
                        db.holidayGreetings[from][mmdd]++;
                        saveDB();
                        setTimeout(() => {
                            const st = getStyle();
                            sock.sendMessage(from, { text: `${st.title}\n${st.divider}\n\n🌸 ✨ *Happy ${ZIM_HOLIDAYS[mmdd]}!* ✨ 🌸\n\n_MINIMA V13 wishes you a wonderful and blessed holiday! 🇿🇼${st.heart}_\n\n${st.divider}`}).catch(() => {});
                        }, 1500);
                    }
                }
            }

            // 🛡️ ANTI-LINK (3 Strikes)
            if (isGroup && db.secDB.antiLink[from] && !isOwner && !userAdmin) {
                const fullText = (body + ' ' + (m.message?.extendedTextMessage?.matchedText || '') + ' ' + (m.message?.extendedTextMessage?.canonicalUrl || '')).toLowerCase();
                if (fullText.includes('chat.whatsapp.com') || fullText.includes('wa.me/') || fullText.match(/https?:\/\/|www\./gi)) {
                    const effectiveBotJid = botJid || sock.user.id.split(':')[0] + '@s.whatsapp.net';
                    const botIsAdmin = await isAdmin(sock, from, effectiveBotJid).catch(() => false);
                    
                    await sock.sendMessage(from, { delete: m.key }).catch(() => {});
                    
                    if (!db.warnDB[from]) db.warnDB[from] = {};
                    if (!db.warnDB[from][sender]) db.warnDB[from][sender] = 0;
                    db.warnDB[from][sender]++;
                    saveDB();
                    
                    const strikes = db.warnDB[from][sender];
                    if (strikes >= 3 && botIsAdmin) {
                        await sock.groupParticipantsUpdate(from,[sender], 'remove').catch(() => {});
                        await sock.sendMessage(from, { text: `🛡️ *Anti-Link:* @${senderNum} was removed for posting links 3 times.`, mentions:[sender] }).catch(() => {});
                        delete db.warnDB[from][sender];
                        saveDB();
                    } else {
                        await sock.sendMessage(from, { text: `🛡️ *Anti-Link Warning ${strikes}/3:* @${senderNum}, links are not allowed here!`, mentions:[sender] }).catch(() => {});
                    }
                    return; // Intercept command processing if link detected
                }
            }

            // Cache for Anti-Delete
            const msgSize = m.message?.imageMessage?.fileLength || m.message?.videoMessage?.fileLength || m.message?.audioMessage?.fileLength || 0;
            if (msgSize <= MAX_ANTIDELETE_SIZE) {
                msgCache.set(m.key.id, { sender, from, pushName, timestamp: Date.now(), message: m.message, body, fileSize: msgSize });
            }

            // AFK System Return
            if (afkDB[sender]) {
                const elapsed = Math.floor((Date.now() - afkDB[sender].time) / 60000);
                delete afkDB[sender];
                await sock.sendMessage(from, { text: `👋 Welcome back @${sender.split('@')[0]}! You were AFK for *${elapsed} min*.`, mentions: [sender] }).catch(() => {});
            }
            ...
            for (const mentioned of mentionedJids) {
                if (afkDB[mentioned]) {
                    ...
                    await sock.sendMessage(from, { text: `😴 @${mentioned.split('@')[0]} is *AFK*\n*Reason:* ${reason}\n*Since:* ${elapsed} min ago`, mentions: [mentioned] }).catch(() => {});
                }
            }

            // Gift logic - Reset to requested Default
            if (!isGroup && !isOwner && !firstDMers.has(sender) && !body.startsWith(config.prefix)) {
                firstDMers.add(sender);
                setTimeout(() => {
                    const st = getStyle();
                    sock.sendMessage(from, { image: { url: st.img }, caption: `✨ *Hey ${pushName}! Welcome to MINIMA V13* ✨\n\n💝 _I have a little surprise gift for you..._\n\n🎁 *Tap here to unwrap:* ${GIFT_LINK}\n\nType *${config.prefix}minima* to see all commands 🌸` }).catch(() => {});
                }, 1500);
            }

            // ── COMMAND HANDLER WITH STYLING ENGINE ──
            if (body.startsWith(config.prefix)) {
                ...
                if (db.secDB.blacklisted.includes(sender) && !isOwner) {
                    return sock.sendMessage(from, { text: '🚫 You are banned from using this bot.' }, { quoted: m });
                }

                if (command.ownerOnly && !isOwner) {
                    return sock.sendMessage(from, { text: '⛔ This command is for the Owner only.' }, { quoted: m });
                }
                if (command.masterOnly && !isMasterOwner) {
                    return sock.sendMessage(from, { text: '👑 This command is strictly reserved for the Master Owner.' }, { quoted: m });
                }
                if (command.adminOnly && !userAdmin && !isOwner) {
                    return sock.sendMessage(from, { text: '👥 This command is for Group Admins only.' }, { quoted: m });
                }
                // groupOnly blocks non-groups — but channel is allowed if owner is running it
                if (command.groupOnly && !isGroup && !isChannel) {
                    return sock.sendMessage(from, { text: '👥 This command only works in groups.' }, { quoted: m });
                }
                if (command.groupOnly && isChannel && !isOwner) {
                    return sock.sendMessage(from, { text: '👑 Only the owner can run this in a channel.' }, { quoted: m });
                }

                const st = getStyle();

                // Custom context reply wrapper (applies Girly/Dark aesthetics to text responses)
                // FIXED: Commands that already contain their own ━━━ formatting header
                // were being double-wrapped. Now only wraps if text doesn't already have
                // its own section divider. rawReply added for unstyled sends.
                const ctx = {
                    sock, m, from, sender, body, args, isGroup, isChannel, isOwner, pushName,
                    reply: async (text) => {
                        // If the text already has its own formatting, don't add the full header
                        const alreadyFormatted = text.includes('━━━') || text.includes('╔') || text.includes('┌') || text.includes('│');
                        const styledText = alreadyFormatted
                            ? `${text}\n\n${st.divider}\n> ${st.sparkle} © ${config.ownerName} | ${st.botName}`
                            : `${st.title}\n${st.divider}\n\n${text}\n\n${st.divider}\n> ${st.sparkle} © ${config.ownerName} | ${st.botName}`;
                        return sock.sendMessage(from, { text: styledText }, { quoted: m });
                    },
                    rawReply: async (text) => sock.sendMessage(from, { text }, { quoted: m }),
                    react: async (emoji) => sock.sendMessage(from, { react: { text: emoji, key: m.key } }).catch(() => {})
                };

                try {
                    // Start reaction without blocking (super fast execution)
                    ctx.react('⏳').catch(() => {});

                    const usr = getUser(sender);
                    usr.commands++;
                    usr.xp += XP_PER_CMD;
                    if (usr.xp >= xpForLevel(usr.level)) {
                    ...
                    // End reaction
                    ctx.react(st.sparkle === '⚠️' ? '🩸' : '✅').catch(() => {});
                } catch (err) {
                    console.error(`❌ Error in [${commandName}]:`, err);
                    ctx.react('❌').catch(() => {});
                    await ctx.reply(`🔄 Hiccup in *${commandName}*. Try again.`).catch(() => {});
                }
            }
        });
        ...
        sock.ev.on('messages.update', async (updates) => {
            for (const { key, update } of updates) {
                try {
                    ...
                    if (!chatId || !db.antiEditGroups[chatId]) continue;
                    const editedMsg = update?.message?.editedMessage?.message || update?.editedMessage?.message || update?.message;
                    if (!editedMsg) continue;
                    const cached = msgCache.get(key.id);
                    if (!cached) continue;
                    const original = extractMessageText(cached.message);
                    const edited   = extractMessageText(editedMsg);
                    if (!original || original === edited) continue;
                    await sock.sendMessage(chatId, { text: `✏️ *MESSAGE EDITED*\n━━━━━━━━━━━━━━━━━━━\n👤 *Sender:* ${cached.pushName}\n━━━━━━━━━━━━━━━━━━━\n📤 *Original:* ${original}\n✏️ *Edited To:* ${edited}\n━━━━━━━━━━━━━━━━━━━` });
                } catch (_) {}
            }
        });

        // ── ANTI-DELETE (Robust recovery fix) ──
        // FIXED: Now recovers video, audio, sticker, and documents — not just image+text.
        // All sends wrapped in .catch() so a closed group or bot removal never crashes the listener.
        sock.ev.on('messages.delete', async (item) => {
            try {
                if (!item.keys || !Array.isArray(item.keys)) return;
                for (const deletedKey of item.keys) {
                    try {
                        const cached = msgCache.get(deletedKey.id);
                        if (!cached) continue;
                        const chatId = cached.from;
                        if (!db.antiDeleteGroups[chatId]) continue;
                        const st = getStyle();
                        const header = `🗑️ *${st.title} - ANTI-DELETE* 🗑️\n${st.divider}\n👤 *Sender:* ${cached.pushName}\n${st.divider}`;
                        ...
                        if (msg?.imageMessage) {
                            try {
                                ...
                                await sock.sendMessage(chatId, { image: buffer, caption: `${header}\n💬 *Caption:* ${cached.body || 'None'}` }).catch(() => {});
                            } catch { await sock.sendMessage(chatId, { text: `${header}\n🖼️ *Image was deleted* (could not recover)` }).catch(() => {}); }

                        } else if (msg?.videoMessage) {
                            try {
                                const fileSize = msg.videoMessage.fileLength || 0;
                                if (fileSize > MAX_ANTIDELETE_SIZE) {
                                    await sock.sendMessage(chatId, { text: `${header}\n🎬 *Video was deleted* ⚠️ Too large to recover (${(fileSize/1024/1024).toFixed(1)}MB)` }).catch(() => {});
                                } else {
                                    const stream = await downloadContentFromMessage(msg.videoMessage, 'video');
                                    let buffer = Buffer.from([]); for await (const chunk of stream) buffer = Buffer.concat([buffer, chunk]);
                                    await sock.sendMessage(chatId, { video: buffer, mimetype: 'video/mp4', caption: `${header}\n🎬 *Video was deleted*\n${msg.videoMessage.caption || ''}` }).catch(() => {});
                                }
                            } catch { await sock.sendMessage(chatId, { text: `${header}\n🎬 *Video was deleted* (could not recover)` }).catch(() => {}); }

                        } else if (msg?.audioMessage) {
                            try {
                                const stream = await downloadContentFromMessage(msg.audioMessage, 'audio');
                                let buffer = Buffer.from([]); for await (const chunk of stream) buffer = Buffer.concat([buffer, chunk]);
                                await sock.sendMessage(chatId, { text: header + '\n🎵 *Audio/Voice note was deleted*' }).catch(() => {});
                                await sock.sendMessage(chatId, { audio: buffer, mimetype: 'audio/mpeg', ptt: msg.audioMessage.ptt || false }).catch(() => {});
                            } catch { await sock.sendMessage(chatId, { text: `${header}\n🎵 *Audio was deleted* (could not recover)` }).catch(() => {}); }

                        } else if (msg?.stickerMessage) {
                            try {
                                const stream = await downloadContentFromMessage(msg.stickerMessage, 'sticker');
                                let buffer = Buffer.from([]); for await (const chunk of stream) buffer = Buffer.concat([buffer, chunk]);
                                await sock.sendMessage(chatId, { sticker: buffer }).catch(() => {});
                                await sock.sendMessage(chatId, { text: header + '\n🎭 *Sticker was deleted*' }).catch(() => {});
                            } catch { await sock.sendMessage(chatId, { text: `${header}\n🎭 *Sticker was deleted* (could not recover)` }).catch(() => {}); }

                        } else if (msg?.documentMessage) {
                            try {
                                const stream = await downloadContentFromMessage(msg.documentMessage, 'document');
                                let buffer = Buffer.from([]); for await (const chunk of stream) buffer = Buffer.concat([buffer, chunk]);
                                await sock.sendMessage(chatId, { document: buffer, mimetype: msg.documentMessage.mimetype || 'application/octet-stream', fileName: msg.documentMessage.fileName || 'deleted_file', caption: `${header}\n📁 *File was deleted*` }).catch(() => {});
                            } catch { await sock.sendMessage(chatId, { text: `${header}\n📁 *File was deleted* (could not recover)` }).catch(() => {}); }

                        } else if (cached.body) {
                            await sock.sendMessage(chatId, { text: `${header}\n💬 *Message:* ${cached.body}` }).catch(() => {});

                        } else {
                            await sock.sendMessage(chatId, { text: `${header}\n❓ *Unknown message type was deleted*` }).catch(() => {});
                        }
                    } catch (innerErr) {
                        console.error('[Anti-Delete inner]', innerErr.message);
                    }
                }
            } catch (e) { console.error('[Anti-Delete outer]', e.message); }
        });

        // ── CALL BLOCKING ──
        sock.ev.on('call', async (calls) => {
            const s = getSettings('global');
            if (!s.callblock) return;
            for (const call of calls) {
                if (call.status === 'offer') {
                    await sock.rejectCall(call.id, call.from).catch(() => {});
                    await sock.sendMessage(call.from, { text: `📵 *Call Blocked*\n_Auto-reject is enabled. Please send a message instead._` }).catch(() => {});
                }
            }
        });
...
process.on('SIGINT', () => { console.log('👋 MINIMA V13 shutting down...'); process.exit(0); });